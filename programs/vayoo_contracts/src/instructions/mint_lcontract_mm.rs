...
pub fn handle(ctx: Context<MintContractMm>, amount: u64) -> Result<()> {
    ...
    let mut amount_to_send=ctx.accounts.contract_state.contract_limiting_bound_amplitude;
    amount_to_send=amount_to_send.checked_mul(2).unwrap().checked_mul(amount).unwrap();
