...
pub fn handle(ctx: Context<CloseShortUser>, amount: u64,
    ...
    let mut amount_to_free=ctx.accounts.contract_state.limiting_amplitude.checked_mul(amount_bought_back).unwrap();
    ...
    let cpi_ctx = CpiContext::new_with_signer(cpi_program, cpi_accounts_transfer_to_free, signer_seeds);
    ...
    let cpi_accounts = Burn {
        ...
        from: ctx.accounts.vault_scontract_ata.to_account_info(),
        authority: ctx.accounts.user_state.to_account_info(),
    };
    let cpi_program = ctx.accounts.token_program.to_account_info();
    let cpi_ctx = CpiContext::new_with_signer(cpi_program, cpi_accounts, signer_seeds);
    ...
    let user_state = &mut ctx.accounts.user_state;
        // Update User State
        user_state.usdc_collateral_locked_as_user -=amount_bought_back.checked_mul(ctx.accounts.contract_state.limiting_amplitude).unwrap();
...
#[account(mut)]
pub vault_scontract_ata: Box<Account<'info, TokenAccount>>,
