...
pub fn handle(ctx: Context<ShortUser>, amount: u64,
    ...
    let cpi_accounts = MintTo {
        ...
        to: ctx.accounts.vault_scontract_ata.to_account_info(),
        authority: ctx.accounts.contract_state.to_account_info(),
    };
    let cpi_program = ctx.accounts.token_program.to_account_info();
    let cpi_ctx = CpiContext::new_with_signer(cpi_program, cpi_accounts, contract_signer_seeds);
    ...
    let cpi_program = ctx.accounts.token_program.to_account_info();
    let cpi_ctx = CpiContext::new_with_signer(cpi_program, cpi_accounts_transfer_to_locked, signer_seeds);
    token::transfer(cpi_ctx, amount_to_send_tolocked)?;

    let user_state = &mut ctx.accounts.user_state;
    // Update User State
    user_state.usdc_collateral_locked_as_user +=amount.checked_mul(ctx.accounts.contract_state.limiting_amplitude).unwrap();
